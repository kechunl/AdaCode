...
class AdaCodeSRNet(nn.Module):
    def __init__(self,
        ...
        # self.weight_predictor = WeightPredictor_RRDB(
        #                         channel_query_dict[self.codebook_scale[0]],
        #                         self.codebook_scale.shape[0],
        #                         self.weight_softmax,
        #                         self.num_RRDB
    ...
    @torch.no_grad()
    def test_tile(self, input, tile_size=240, tile_pad=16, cat_map=False):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                # upscale tile
                output_tile, weight_tile = self.test(input_tile, cat_map=True)
                ...
                weight_map[:, :, :, output_start_y:output_end_y,
                       output_start_x:output_end_x] = weight_tile[:, :, :, output_start_y_tile:output_end_y_tile,
                                                                  output_start_x_tile:output_end_x_tile]
        if cat_map:
    ...
    @torch.no_grad()
    def test(self, input, cat_map=False):
        ...
        # process weight B x n x 1 x H x W
        if cat_map:
            weight = aux['weight']
            weight = F.pixel_shuffle(weight.repeat(1,1,64,1,1), 8)
            weight = weight[..., :h_old * self.scale_factor, :w_old * self.scale_factor]
            # category_map = torch.argmax(torch.abs(weight), dim=1) # 0,1,...,N-1 (for N categories). MAXIMUM or Absolute Maximum?
            # category_map = F.pixel_shuffle(category_map.repeat(1, 64, 1, 1), 8)
            # category_map = category_map[..., :h_old * self.scale_factor, :w_old * self.scale_factor] # B x 1 x ori_H x ori_W
