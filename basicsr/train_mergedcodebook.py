...
def train_pipeline(root_path):
    ...
    # freeze codebook
    frozen_module_keywords = opt['network_g'].get('frozen_module_keywords', None) 
    ...
    codebook_paths = opt['path'].get('pretrain_codebook', None)
    code_pos = 0
    for codebook_path in codebook_paths:
        load_net = torch.load(codebook_path)
        codebook = load_net['params']['quantize_group.0.embedding.weight']
        model.net_g.module.quantize_group[0].embedding.weight[code_pos:code_pos+codebook.shape[0],:] = codebook
        code_pos += codebook.shape[0]
